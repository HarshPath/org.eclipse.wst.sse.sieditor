...
public abstract class AbstractTreeNode implements ITreeNode {
    ...
    private final TreeNodeMapper nodeMapper;

    /**
     * Do not use this field directly, but do use it via
     * {@link #getCategories()}
     */
    private int categories = 0;

    /**
     * This constructor will assume node category as CATEGORY_MAIN, or
     * CATEGORY_IMPORTED (in case that parent.isImportedNode()). It is required
     * to create a tree node with explicitly set category
     * 
     * @deprecated do use
     *             {@link #AbstractTreeNode(IModelObject, ITreeNode, TreeNodeMapper, int)}
     */
    @Deprecated
    public AbstractTreeNode(final IModelObject modelObject, final ITreeNode parent, final TreeNodeMapper nodeMapper) {
    ...
    /**
     * Create a tree node with specified categories. This node will have these
     * categories and the categories from the parent node.
     * 
     * @see ITreeNode
     */
    public AbstractTreeNode(final IModelObject modelObject, final ITreeNode parent, final TreeNodeMapper nodeMapper,
    ...
    protected abstract ITreeNodeLabelsProviderFactory getLabelsProviderFactory();

    @Override
    public String getDisplayName() {
        return getLabelsProviderFactory().getLabelsProvider(this).getDisplayName();
    }

    @Override
    public String getTypeDisplayText() {
        return getLabelsProviderFactory().getLabelsProvider(this).getTypeDisplayText();
    }

    @Override
    public String getTreeDisplayText() {
        return getLabelsProviderFactory().getLabelsProvider(this).getTreeDisplayText();
    }
    ...
    public boolean isReadOnly() {
        ...
        final int updatedCategories = getCategories();

        if ((updatedCategories & CATEGORY_IMPORTED) != 0 || (updatedCategories & CATEGORY_REFERENCE) != 0) {
            return true;
        }
    ...
    private void updateReferenceCategory() {
        if ((parent != null && (parent.getCategories() & CATEGORY_REFERENCE) != 0) || EmfXsdUtils.isReference(modelObject)) {
            categories |= CATEGORY_REFERENCE;
        } else {
            categories &= ~CATEGORY_REFERENCE;
        }
    ...
    private boolean isImportedNode(final ITreeNode node) {
        final ITreeNode parent = node.getParent();

        if (node instanceof ImportedServicesNode || node instanceof ImportedTypesNode) {
            return true;
        } else if (parent != null) {
