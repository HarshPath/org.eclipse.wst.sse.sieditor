...
import org.eclipse.wst.sse.sieditor.ui.v2.propertyeditor.StructureTypeEditor;
...
public class StructureTypeDetailsSection extends AbstractDetailsPageSection {

    private final StructureTypeEditor typeEditor;
    ...
    public StructureTypeDetailsSection(final IFormPageController controller, final FormToolkit toolkit,
            final IManagedForm managedForm, final ElementNodeDetailsController detailsController,
            final ITypeDisplayer typeDisplayer) {
        super(controller, toolkit, managedForm);
        this.detailsController = detailsController;
        typeEditor = new StructureTypeEditor(controller, typeDisplayer, detailsController);
    ...
    public void createContents(final Composite parent) {
        ...
        initializeControls(toolkit, clientComposite);
    ...
    protected void createControls(final FormToolkit toolkit, final Composite clientComposite) {
        ...
        GridDataFactory.swtDefaults().grab(true, false).span(3, 1).align(SWT.FILL, SWT.CENTER).applyTo(nameText);
        typeEditor.createControl(toolkit, clientComposite);
    }

    private void initializeControls(final FormToolkit toolkit, final Composite clientComposite) {
    ...
    public boolean isDirty() {
        return isNameDirty || typeEditor.isDirty();
    }
    ...
    public boolean isStale() {
        ...
        return !nameTextValue.equals(utils().getDisplayName(type)) || typeEditor.isStale();
    }
    ...
    public void refresh() {
        ...
        final boolean applicable = detailsController.isTypeApplicable();
        if (applicable) {
            typeEditor.setInput(node);
            typeEditor.update();
        }
        typeEditor.setVisible(applicable);
        getProblemDecorator().bind(XSDPackage.Literals.XSD_COMPLEX_TYPE_DEFINITION__BASE_TYPE_DEFINITION, typeEditor);
