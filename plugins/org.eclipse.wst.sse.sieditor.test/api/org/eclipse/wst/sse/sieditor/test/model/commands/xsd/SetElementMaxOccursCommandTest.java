...
public class SetElementMaxOccursCommandTest extends ElementManipulationsAbstractCommandTest {
    private static final String NEW_ELEMENT_NAME = "maxOccursElement";
    ...
    protected void assertPostRedoState(final IStatus redoStatus, final IWsdlModelRoot modelRoot) {
        assertEquals(VALUE, element.getMaxOccurs());
    }

    @Override
    protected void assertPostUndoState(final IStatus undoStatus, final IWsdlModelRoot modelRoot) {
        assertEquals(oldValue, element.getMaxOccurs());
    }

    @Override
    protected AbstractNotificationOperation getOperation(final IWsdlModelRoot modelRoot) throws Exception {
        element = createElement(modelRoot, NEW_ELEMENT_NAME);
        oldValue = element.getMaxOccurs();

        return new SetElementOccurences(modelRoot, element,element.getMinOccurs(), VALUE);
    }
}
