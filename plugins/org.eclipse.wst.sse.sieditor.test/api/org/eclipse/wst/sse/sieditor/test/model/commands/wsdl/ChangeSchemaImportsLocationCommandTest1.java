...
public class ChangeSchemaImportsLocationCommandTest1 extends AbstractCommandTest {

    private static final String NAMESPACE_TO_UPDATE = "http://www.example.org/NewWSDLFile/";
    ...
    protected void assertPostRedoState(final IStatus redoStatus, final IWsdlModelRoot modelRoot) {
        ...
        for (final XSDConcreteComponent component : schema.getComponent().getContents()) {
            if (component instanceof XSDImport && ((XSDImport) component).getNamespace().equals(NAMESPACE_TO_UPDATE)) {
                found = true;
                assertNotNull(((XSDImport) component).getSchemaLocation());
                assertEquals("ExtractedSchema1.xsd", ((XSDImport) component).getSchemaLocation());
                break;
            }
        }
        assertTrue(found);
    ...
    protected AbstractNotificationOperation getOperation(final IWsdlModelRoot modelRoot) throws Exception {
        ...
        return new UpdateSchemasImportCommand(modelRoot, new ISchema[] { schema }, NAMESPACE_TO_UPDATE, new Path(modelRoot
                .getDescription().getLocation()).removeLastSegments(1).append("ExtractedSchema1.xsd"), true);
    }
