 ...
 *******************************************************************************/
package org.eclipse.wst.sse.sieditor.ui.v2.dt.extract.wizard;
...
import org.eclipse.wst.sse.sieditor.ui.v2.dt.extract.wizard.pages.ExtractSchemaWizardPage;
import org.eclipse.wst.sse.sieditor.ui.v2.dt.extract.wizard.pages.SchemaDependenciesWizardPage;
...
public class ExtractNamespaceWizard extends Wizard {
    ...
    public IStatus init(final ISchema schema) {
        try {
            ...
            wsdlLocationPath = new Path(((Description) schema.getRoot()).getRawLocation());
    ...
    public boolean performFinish() {
        this.useAsImport = extractSchemaWizardPage.getUseAsImportCheckBox().getSelection();

        final ExtractNamespaceRunnable runnable = new ExtractNamespaceRunnable(schemaNode, dependenciesSet, useAsImport,
                wsdlLocationPath);
        try {
            wizardDialog.run(false, true, runnable);

        } catch (final InvocationTargetException e) {
            Logger.logError(MessageFormat.format("The extraction of XML Schema \"{0}\" failed",//$NON-NLS-1$ 
