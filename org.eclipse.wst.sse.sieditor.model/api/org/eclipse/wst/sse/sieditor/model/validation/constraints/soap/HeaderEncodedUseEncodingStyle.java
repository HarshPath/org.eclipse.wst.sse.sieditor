...
public class HeaderEncodedUseEncodingStyle extends AbstractConstraint{

	@Override
	protected IStatus doValidate(IValidationContext ctx) {
		SOAPHeaderBase header = (SOAPHeaderBase) ctx.getTarget();
		
		if ("encoded".equals(header.getUse()) && header.getEEncodingStyles().isEmpty()) { //$NON-NLS-1$
			if (header instanceof SOAPHeaderFault) {
				return ConstraintStatus.createStatus(ctx, header, null, Messages.SOAPHeaderEncodedUseEncodingStyle_1);
			}
			return ConstraintStatus.createStatus(ctx, header, null, Messages.SOAPHeaderEncodedUseEncodingStyle_2);
		}
